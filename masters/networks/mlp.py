...
class MLPConfig:
    ...
    # n_hidden: int
    n_out: int
    norm_inpt_out: Optional[float] = 0.5
...
class MLP:
    def __init__(
        ...
        self.n_out = n_out
        self.norm_inpt_out = norm_inpt_out
        ...
        # Build network.
        self.network = Network(dt=1.0, reward_fn=MovingAvgRPE)
        ...
        # Connections
        # inpt_middle = Connection(source=inpt, target=hidden, wmin=0, wmax=1, update_rule=PostPre)
        ...
        input_out = Connection(
            ...
            update_rule=MSTDPET,  # learning rule
            nu=nu,  # learning rate
            norm=norm_inpt_out * inpt.n if norm_inpt_out is not None else None,  # normalization
        ...
        # self.network.add_layer(input_out, name=HIDDEN_LAYER_NAME)
        self.network.add_layer(out, name=OUTPUT_LAYER_NAME)
        self.network.add_connection(input_out, source=INPUT_LAYER_NAME, target=OUTPUT_LAYER_NAME)
